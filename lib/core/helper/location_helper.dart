...
import 'package:presensi_pegawai/core/helper/dialog_helper.dart';
...
class LocationHelper{
  ...
  static Future<bool> showDialogLocationPermission(BuildContext context) async{
    ...
    if(permission == LocationPermission.denied || permission == LocationPermission.deniedForever){
      ...
      if(permission == LocationPermission.denied || permission == LocationPermission.deniedForever){
          DialogHelper.showSnackbar(context: context, message: 'Perizinan Lokasi Ditolak');
          return false;
      }  
    }
  ...
  static bool isLocationInCircle (CircleOSM circle, GeoPoint currentLocation){
    double distance = Geolocator.distanceBetween(currentLocation.latitude, currentLocation.longitude, circle.centerPoint.latitude, circle.centerPoint.longitude);
